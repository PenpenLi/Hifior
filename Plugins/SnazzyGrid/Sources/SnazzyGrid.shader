...
//          
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_392 = ((float2(node_371.g,node_371.b)/_Scale)+float2(_SnapOffset.y,_SnapOffset.z));
                float4 node_372 = i.posWorld;
                float2 node_396 = ((float2(node_372.r,node_372.b)/_Scale)+float2(_SnapOffset.x,_SnapOffset.z));
                float4 node_373 = i.posWorld;
                float2 node_400 = ((float2(node_373.r,node_373.g)/_Scale)+float2(_SnapOffset.x,_SnapOffset.y));
                float node_1707 = 0.5;
                ...
                float2 node_496 = ((float2(node_484.g,node_484.b)/_GridSteps)+float2(_GridOffset.y,_GridOffset.z));
                float4 node_486 = i.posWorld;
                float2 node_500 = ((float2(node_486.r,node_486.b)/_GridSteps)+float2(_GridOffset.x,_GridOffset.z));
                float4 node_488 = i.posWorld;
                float2 node_504 = ((float2(node_488.r,node_488.g)/_GridSteps)+float2(_GridOffset.x,_GridOffset.y));

			    float termA=(tex2D(_SnazzyGrid1,TRANSFORM_TEX(node_1735.rg, _SnazzyGrid1)).b*lerp(lerp(((node_511.r*tex2D(_SnazzyGrid2,TRANSFORM_TEX(node_496, _SnazzyGrid2)).r + node_511.g*tex2D(_SnazzyGrid2,TRANSFORM_TEX(node_500, _SnazzyGrid2)).r + node_511.b*tex2D(_SnazzyGrid2,TRANSFORM_TEX(node_504, _SnazzyGrid2)).r)*_GridColor.a),node_64,node_64),node_1221,node_1317));
    			float termB=saturate(pow(dot((i.normalDir), UNITY_MATRIX_V[2].xyz),_Fresnel)*1.1-0.1);
    			float alpha=termA*termB;
