...
public static class LinqS
{
    ...
    public static int IndexOfMax<T>(IEnumerator<T> a) where T : IComparable
    {
        int i = 0, max_index = 0;
        a.Reset();
        a.MoveNext();
    ...
    public static int IndexOfMin<T>(IEnumerator<T> a) where T : IComparable
    {
        int i = 0, max_index = 0;
        a.Reset();
        bool hasNext = a.MoveNext();
        if (!hasNext) return -1;
        T max = a.Current;
