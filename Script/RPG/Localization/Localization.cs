...
public static class Localization
{
    ...
    public static GameObject UIRoot
    {
        get
        {
            if (_uiRoot == null)
            {
                ...
                UnityEngine.Assertions.Assert.IsNotNull(_uiRoot, "请确保场景中至少存在一个UICanva");
            }
            return _uiRoot;
        }
    }
    ...
    static bool LoadCSV(byte[] bytes, TextAsset asset, bool merge = false)
    {
        ...
        List<string> header = reader.ReadCSV();
        ...
        {
            mDictionary.Clear();
            mLanguages = new string[header.Count];
        ...
        else
        {
            ...
            for (int i = 0; i < header.Count; ++i) languagesToAdd[i] = header[i];
    ...
    static void AddCSV(List<string> newValues, string[] newLanguages, Dictionary<string, int> languageIndices)
    {
    ...
    static string[] ExtractStrings(List<string> added, string[] newLanguages, Dictionary<string, int> languageIndices)
    {
    ...
    static bool SelectLanguage(string language)
    {
        ...
        for (int i = 0, imax = mLanguages.Length; i < imax; ++i)
        {
            if (mLanguages[i] == language)
            {
                ...
                foreach (IModifyLanguageHandle t in UIRoot.GetComponentsInChildren<IModifyLanguageHandle>())
                    t.OnLocalize();
                //UnityEngine.EventSystems.ExecuteEvents.Execute<IModifyLanguageHandle>(UIRoot, null, (x, y) => { x.OnLocalize(); });
                //UIRoot.BroadcastMessage("OnLocalize");
                return true;
            }
        }
        return false;
