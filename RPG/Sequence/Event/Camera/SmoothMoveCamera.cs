...
namespace Sequence
{
    public class SmoothMoveCamera : SequenceEvent
    {
        ...
        /// </summary>
        public Camera mainCutsceneCam;/// <summary>
        ...
        private void CurveMove()
        {
            if (currentUseLerp) //If we are lerping, we need to create the points as we go
            {
                ...
                if (mobileCurve)
                {
                    mainCamT.position = Bezier2(t, cutsceneCameraPoints[currentEvent + 1].position, cutsceneMidPoints[currentEvent + 1].position,
                                                         cutsceneCameraPoints[currentEvent + 2].position);
                }
                else {
                    mainCamT.position =
                        Bezier3(t, cutsceneCameraPoints[currentEvent + 1].position, cutsceneMidPoints[currentEvent + 1].position,
                                         cutsceneCubicMidPoints[currentEvent + 1].position,
                                         cutsceneCameraPoints[currentEvent + 2].position);
                }
            ...
            if (mobilePath)
            {
                ...
                for (int i = 0; i < curveNodeCount[currentIndex]; i++)
                {
                    if (debugPath.Length > i)
                        debugPath[i] =
                            Bezier2((float)i / curveNodeCount[currentIndex], cutsceneCameraPoints[currentIndex + 1].position, cutsceneMidPoints[currentIndex + 1].position,
                ...
                for (int i = 0; i < curveNodeCount[currentIndex]; i++)
                {
                    if (debugPath.Length > i)
                        debugPath[i] =
                            Bezier3((float)i / curveNodeCount[currentIndex], cutsceneCameraPoints[currentIndex + 1].position, cutsceneMidPoints[currentIndex + 1].position,
